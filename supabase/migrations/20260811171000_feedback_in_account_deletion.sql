...
--    пути он собирает ДО вызова, после обнуления брать их уже неоткуда.
--
...
begin
  ...
  select coalesce(array_agg(x), '{}') into v_paths from (
    ...
    -- Скриншот отзыва: единственный указатель на файл, и через несколько строк
    -- его снесёт каскад. Не собрать здесь - значит потерять файл навсегда.

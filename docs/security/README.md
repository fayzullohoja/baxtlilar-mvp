...
Итого к работе остаётся: IP-allowlist (A2, сперва проверить сервер) и массовый
разлогин админов (A3). A4 закрыт. A1 отложен решением владельца 18.08.2026 -
обязательный второй фактор пока не делаем, вопрос закрыт до его пересмотра.

...
export const ADMIN_ERROR_RU: Record<string, string> = {
  ...
  // Коды отмены блокирующего отказа (/unblock-verification). Без них оператор
  // видел бы сырой код латиницей вместо объяснения, что делать дальше.
  banned_lifecycle:
    "Пользователь забанен или удалён - сначала разбаньте, потом откатывайте блокировку верификации.",
  wrong_lifecycle:
    "Пользователь забанен или удалён - сначала разбаньте, потом откатывайте блокировку верификации.",
  not_rejected: "Верификация не отклонена - откатывать нечего.",
  not_blocking:
    "Отказ был техническим, а не блокирующим - откатывать нечего: человек перезаливает документы сам.",
  no_documents: "У пользователя нет загруженных документов.",

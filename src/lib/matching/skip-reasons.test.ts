...
describe("причины отказа и срок возврата", () => {
  ...
  it("жалоба на возрастные рамки прячет бессрочно - её снимает правка, а не время", () => {
    expect(hiddenUntil("age", NOW)).toBeNull();
  });

  it("гео прячет НЕ навсегда: снять такой отказ правкой нечем", () => {
    // geo_preference жёстко выставлен в 'my_city' при заполнении анкеты и не
    // редактируется нигде. Бессрочное сокрытие обещало бы починку, которой нет,
    // и молча выгрызало ленту, где на человека приходится девять кандидатов.
    expect(hiddenUntil("geo", NOW)!.getTime()).toBe(NOW + 90 * DAY);
    expect(suggestsFilterFix("geo")).toBe(false);
  });

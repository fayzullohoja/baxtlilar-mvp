...
/**
 * Что предлагаем в АНКЕТЕ. Замечание 6 тестеров family launch: вариант «Другое»
 * убран из выбора. Он ничего не сообщал о человеке, но при этом продолжал
 * тянуть за собой вопросы «сколько детей хотите» и «когда планируете брак» -
 * то есть человек отвечал «не скажу», а его всё равно расспрашивали дальше.
 *
 * Из MARITAL_STATUS значение НЕ удалено намеренно: тем же массивом питается
 * проверка zod и редактор в админке, а у части людей 'other' уже записан.
 * Убери его оттуда - и у этих людей сломается сохранение анкеты, а модератор
 * перестанет видеть их настоящий статус. Поэтому список выбора отдельный:
 * новые люди этот вариант не увидят, старые продолжают работать.
 */
export const MARITAL_STATUS_SELECTABLE: Opt[] = MARITAL_STATUS.filter(
  (o) => o.value !== "other",
);

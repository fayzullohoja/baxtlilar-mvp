...
function cautionsFor(
  ...
  if (relaxLevel > 0) {
    // Замечание 10 тестеров family launch: человек поставил 25-32, увидел людей
    // вне этого диапазона и решил, что фильтр сломан. Фильтр работает - это
    // лестница ослабления из миграции 20260703000000: на уровне 1 диапазон
    // раздвигается на пять лет в обе стороны (нижняя граница не опускается
    // ниже 18), на уровне 2 дополнительно снимается встречное условие.
    //
    // Прежний текст говорил «немного расширили» и не называл чисел, поэтому
    // вопрос «почему мне показывают 37-летнего, я просил до 32» оставался без
    // ответа. Теперь называем и исходный диапазон, и фактический.
    const from = viewer.partner_age_min;
    const to = viewer.partner_age_max;
    if (typeof from === "number" && typeof to === "number") {
      const effFrom = Math.max(18, from - 5);
      const effTo = to + 5;
      out.push(
        `Вы искали ${from}-${to}, но подходящих в этих возрастных рамках сейчас ` +
          `нет. Мы расширили поиск до ${effFrom}-${effTo}, чтобы показать Вам ` +
          `этого человека.`,
      );
    } else {
      out.push(
        "Подходящих в Ваших возрастных рамках сейчас нет, поэтому мы расширили поиск.",
      );
    }
  }

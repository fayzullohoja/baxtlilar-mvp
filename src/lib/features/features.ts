 ...
 *
 * У invite_gate смысл ПЕРЕВЁРНУТ: true значит "шлагбаум поднят, вход открыт",
 * а дефолт false значит "закрыт". Fail-open к дефолту здесь означало бы
 * "не знаем состояние защиты - предположим, что она выключена" - то есть
 ...
 * обязано читаться как "закрыто": перечисленные здесь флаги на сбое чтения
 * принудительно становятся true, ЧТО БЫ НИ ГОВОРИЛ их дефолт.
 */

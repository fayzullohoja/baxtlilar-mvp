...
export const SET_RETURNING = new Set([
  ...
  // create_feedback (returns table(feedback_id, limited)): без записи здесь
  // роут читал бы composite СТРОКОЙ "(uuid,f)", не смог бы отличить успех от
  // лимита и отвечал бы 500 на уже сохранённый отзыв.
  "create_feedback",
]);

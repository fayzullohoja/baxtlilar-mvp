...
describe("extractInviteCode", () => {
  ...
  it("извлекает код, разбитый пробелом (латиница)", () => {
    expect(extractInviteCode("7K2 MQX")).toBe("7K2MQX");
  });

  it("извлекает код, разбитый пробелом (кириллица)", () => {
    // Кириллический код, разбитый пробелом, тоже должен работать.
    // "ВХТ 7К2" нормализуется в "BXT7K2" (6 символов с цифрой).
    expect(extractInviteCode("ВХТ 7К2")).toBe("BXT7K2");
  });
  ...
  describe("слова-ловушки: цифра отличает код от обычного слова", () => {
    it("отклоняет русское слово-ловушку перед кодом (Секрет: 7K2MQX)", () => {
      // "Секрет" без цифры, поэтому игнорируется. Находим "7K2MQX" с цифрой.
      expect(extractInviteCode("Секрет: 7K2MQX")).toBe("7K2MQX");
    });

    it("отклоняет русское слово-ловушку (Маркет 7K2MQX)", () => {
      // "Маркет" даёт "MAPKET" (6 символов), но БЕЗ цифры, поэтому отклоняется.
      expect(extractInviteCode("Маркет 7K2MQX")).toBe("7K2MQX");
    });

    it("отклоняет узбекское слово-ловушку (Qanday 7K2MQX)", () => {
      // "Qanday" даёт 6 символов, но БЕЗ цифры - отклоняется.
      // Узбекский язык пишется латиницей, поэтому это важный тест.
      expect(extractInviteCode("Qanday 7K2MQX")).toBe("7K2MQX");
    });

    it("отклоняет английское слово-ловушку (Market 7K2MQX)", () => {
      // "Market" даёт 6 символов, но БЕЗ цифры - отклоняется.
      expect(extractInviteCode("Market 7K2MQX")).toBe("7K2MQX");
    });

    it("работает с кириллическим кодом в фразе", () => {
      // Код на кириллице (без окружающих слов-ловушек) находится по цифре.
      expect(extractInviteCode("Держи ВХТЕКМ заходи")).toBe("");
      // Но если добавить цифру, находится.
      expect(extractInviteCode("Держи ВХТ7КМ заходи")).toBe("BXT7KM");
    });

    it("отклоняет обе слова-ловушки без цифры (Секрет Маркет)", () => {
      // Оба слова дают 6 символов, но БЕЗ цифр.
      // extractInviteCode вернёт пустую строку.
      // Мутация БЕЗ проверки цифры вернула бы первое слово.
      expect(extractInviteCode("Секрет Маркет")).toBe("");
    });
  });
});
...
describe("generateInviteCode", () => {
  ...
  it("каждый код содержит минимум одну цифру и минимум одну букву", () => {
    for (let i = 0; i < 200; i++) {

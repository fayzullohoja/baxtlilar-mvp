...
/**
 * Погасить все активные коды пользователя (бан, утечка, ручное действие) и -
 * кроме reason="ban" - закрыть его персональное право получить НОВЫЙ код
 * (users.invite_revoked_at). См. развёрнутый комментарий в ensureCodeForUser
 * про то, зачем это второе поле вообще существует.
 *
 * ⛔ Раунд исправлений 1 (независимое ревью Task 10): раньше здесь было ДВА
 * последовательных UPDATE через supabase-клиент - сначала гасили строку кода,
 * потом ставили запрет. Если первый проходил, а второй падал (сеть, таймаут,
 * состязание за users - самую горячую таблицу проекта), получалось: строка
 * кода мертва, а право приглашать - НЕТ. Функция при этом честно бросала (см.
 * старые комментарии/тесты), но было поздно - первый UPDATE УЖЕ закоммичен, и
 * откатить его нечем. Итог: оператор видит ошибку, полагает что ничего не
 * изменилось, а на деле человек остался с правом приглашать - дословно та
 * дыра, ради которой вся задача. Простая перестановка порядка тоже не лечит:
 * тогда при сбое ВТОРОГО шага строка кода осталась бы активной, и утёкшим
 * кодом смогли бы воспользоваться посторонние - хуже, а не лучше.
 *
 * Лечится ТОЛЬКО транзакцией: обе записи перенесены в SQL-функцию
 * disable_invite_codes_of_user (миграция 20260811160000) - несколько
 * UPDATE внутри одного plpgsql-тела выполняются в ОДНОЙ транзакции Postgres
 * автоматически, по образцу admin_ban_confirm (несколько записей в одном
 * вызове). Если тело падает на любом шаге - откатывается ВСЁ целиком, включая
 * уже выполненные внутри этого же вызова операторы. Здесь остаётся один RPC-
 * вызов вместо двух прямых UPDATE.
 */
export async function disableCodesOfUser(userId: string, reason: string): Promise<void> {
  const { data, error } = await supabaseAdmin().rpc("disable_invite_codes_of_user", {
    p_user_id: userId,
    p_reason: reason,
  });
  // Гашение - рычаг безопасности (бан, утечка кода). Тихий провал здесь значит:
  // код остаётся активным (или право приглашать тихо не закрылось), а оператор
  // в это время видит "готово". Бросаем, чтобы вызывающий роут вернул честную
  // ошибку вместо ложного успеха. Транзакция внутри RPC гарантирует, что при
  // ЛЮБОМ сбое (транспортном - error, или логическом внутри функции - !ok)
  // состояние в БД осталось РОВНО тем, что было до вызова - НИ ОДНА из двух
  // записей не применилась наполовину.
  if (error) throw new Error(`не удалось погасить коды пользователя: ${error.message}`);
  const r = data as { ok: boolean; error?: string } | null;
  if (!r?.ok) throw new Error(`не удалось погасить коды пользователя: ${r?.error ?? "неизвестная ошибка функции БД"}`);
}

...
Локальная копия для чтения: `~/Desktop/Все файлы/17. Baxtlilar/` - структура повторяет докстор
(`01. Спека (Чаты)`, `02. PRD, бэклог и схемы`, `03. Договоры и акты`), см. README в корне папки.
Папку периодически перекладывают, поэтому ищи по имени файла, а не по жёсткому пути. Имена в NFD-
нормализации: `ls` и glob промахиваются по путям с й/ё, надёжно - `os.listdir` со сверкой NFC.
